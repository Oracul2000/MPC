...
for step in range(12000):
    ...
    v = current_state.vx  # продольная скорость

    # === Lateral control (Pure Pursuit) ===
    steering = pp.compute_steering(x, y, yaw, v)

    # === Longitudinal control (PID по скорости) ===
    ...
    # === Вычисление Cross-Track Error (CTE) ===
    ...
    # Сохраняем данные (каждые 100 шагов, чтобы не перегружать память)
    if step % 100 == 0:
