...
# Генерация синусоидальной траектории вместо double_P
x_cl = np.linspace(0, 200, 1000)  # X от 0 до 200 метров
y_cl = 10 * np.sin(2 * np.pi * x_cl / 50)  # Амплитуда 10 м, период 50 м
s_cl = compute_arc_length(x_cl, y_cl)
kappa_cl = compute_curvature(x_cl, y_cl, s_cl)
kappa_fun = build_kappa_function(s_cl, kappa_cl)
dt_sim = 0.001
mpc = FrenetMPC(kappa_fun, N=50, dt=0.05)
...
car.set_initial_state(yaw=0.0)  # Начальный yaw=0 для движения вдоль X
time_vec = []
t = 0.0
...
ax.plot(x_cl, y_cl, 'k--', label='Центральная линия (синусоида)')
...
ax.set_xlabel('X [м]')
ax.set_ylabel('Y [м]')
ax.set_title('Визуализация движения машины с MPC по синусоиде')
ax.axis('equal')
ax.grid(True)
ax.legend()
...
anim.save('mpc_sine_animation.gif', writer='pillow')

# Статичные графики (как раньше)
plt.figure(figsize=(12, 10))
plt.subplot(4, 1, 1)
plt.plot(x_cl, y_cl, 'k--', label='Центральная линия (синусоида)')
plt.plot(x_traj, y_traj, 'r-', linewidth=2, label='MPC')
plt.ylabel('Y [м]')
plt.xlabel('X [м]')
plt.legend()
plt.title('Следование траектории по синусоиде')
plt.axis('equal')
plt.grid(True)
plt.subplot(4, 1, 2)
